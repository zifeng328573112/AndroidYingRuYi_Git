...
public class PriceView extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.priceview);
		...
		priceListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (listData.size() > position) {
					...
					Intent in = new Intent(PriceView.this, DetailTab.class);
	...
	private synchronized void updateData(String type) {
		// logger.debug("发出数据请求" + new Date().toLocaleString());
		listData = util.getPriceData("http://m.fx678.com/quotelist.aspx?key="
				+ type, type);
	}

	public synchronized void showNew() {
		...
		for (int i = 0; i < mData.size(); i++) {
			...
			if (code.equals(timeNow.getPrice_code())) {
				...
				map.put(Const.PRICE_LASTCLOSE, timeNow.getPrice_lastclose());
				...
				map.put(Const.PRICE_VOLUME, timeNow.getPrice_volume());
				map.put(Const.PRICE_AVERAGE, timeNow.getPrice_average());
				map.put(Const.PRICE_TURNOVER, timeNow.getPrice_turnover());
	...
	public void showData() {
		...
		for (int i = 0; i < listData.size(); i++) {
			...
			map.put(Const.PRICE_AVERAGE, pData.getPrice_average());
			mData.add(map);
		...
		MyAdapter adapter = (MyAdapter) priceListView.getAdapter();
		adapter.notifyDataSetChanged();
		// logger.debug("显示完毕" + new Date().toLocaleString());
	}
	...
	private class MyAdapter extends BaseAdapter {
		...
		public View getView(int paramInt, View paramView,
			...
			if (paramView == null) {
				...
				vholder.average = (TextView) paramView
						.findViewById(R.id.average);
				vholder.turnover = (TextView) paramView
			...
			vholder.settle.setText((String) mData.get(paramInt).get(
					Const.PRICE_LASTSETTLE));
			...
			vholder.volume.setText((String) mData.get(paramInt).get(
					Const.PRICE_VOLUME));
			vholder.average.setText((String) mData.get(paramInt).get(
					Const.PRICE_AVERAGE));
			String turnovertmp = (String) mData.get(paramInt).get(
					Const.PRICE_TURNOVER);
			...
			while (running) {
				...
				try {
					...
					if (str.indexOf("{") >= 0 && str.indexOf("}") >= 0) {
						timeNow = util.getTimeNowUDPString(str, null);
						if (timeNow != null) {
							Message msg = mHandler.obtainMessage();
							Bundle b = new Bundle();
							b.putInt("total", 4);
	...
	protected void authorDialog() {
		AlertDialog.Builder builder = new Builder(PriceView.this);
